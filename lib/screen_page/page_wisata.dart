...
class _PageWisataState extends State<PageWisata> {
  ...
  Future<void> fetchWisata() async {
    final response = await http.get(Uri.parse('http://192.168.18.8/wisataDB/getWisata.php'));
  ...
  Future<void> deleteWisata(String id) async {
    final response = await http.post(
      Uri.parse('http://192.168.18.8/wisataDB/deleteWisata.php'),
      body: {'id': id},
    );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Daftar Wisata Sumbar'),
          ...
          return Card(
            ...
            child: ListTile(
              leading: wisata.gambar.isNotEmpty
                  ? Image.network(
                'http://192.168.18.8/wisataDB/gambar/${wisata.gambar}',
                width: 50,
                height: 50,
                fit: BoxFit.cover,
              )
